...
namespace PixelCorrection_Spinnaker_WPF
{
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            InitializeComponent();
            Title = string.Format("SpinPixelCorrection Utility. Tier {0}", (RenderCapability.Tier >> 16));
        ...
        public void updateCamListView()
        {
            ...
            foreach(IManagedCamera managedCamera in camList)
            {
                try
                {
                    // Nodemap information to get device information.
                    INodeMap nodeMapTLDevice = managedCamera.GetTLDeviceNodeMap();
                    managedCamera.Init();

                    // Grab camera
                    INodeMap nodeMap = managedCamera.GetNodeMap();
                    ICategory deviceInfo = nodeMapTLDevice.GetNode<ICategory>("DeviceInformation");
                    ...
                    {
                        string deviceModelName = deviceInfo.Children[3].ToString();
                        string deviceSerialNumber = deviceInfo.Children[0].ToString();
                        string deviceDefectCorrectEnable = iDefectCorrectStaticEnable.Value.ToString();
                        string deviceType = deviceInfo.Children[4].ToString();
            ...
            try
            {
                ...
                if(selectedIdx >= 0)
                {
                    result = PCGlobal.Remove(
                        ref PCCamInfoGlobal, ref PCDefectPixelListGlobal, listView.Items.IndexOf(listView.SelectedItem));
